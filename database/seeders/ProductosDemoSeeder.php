...
class ProductosDemoSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->command->info('✅ ProductosDemoSeeder finalizado. Datos confirmados en la BD de prueba.');
    }
    ...
    protected function sembrarProductos(array $marcas, array $categorias): array
    {
        ...
        foreach (static::CONFIG_PRODUCTOS as $configKey => $cfg) {
            ...
            for ($i = 0; $i < $cfg['cantidad']; $i++) {
                ...
                $marcasValidas = array_filter($cfg['marcas'], fn($m) => isset($marcas[$m]));
                if (empty($marcasValidas)) {
                    continue;
                }
                
                // ── Nombre y slug únicos ────────────────────────────────────
                $marcaSlug = $marcasValidas[array_rand($marcasValidas)];
