        ...
        <div class="flex flex-col items-center justify-center gap-2.5 text-center w-full">
            ...
            <div class="flex justify-center">
                <span class="inline-flex rtl:flex-row-reverse shadow-2xs rounded-xl overflow-hidden border border-slate-200">
                    ...
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-slate-700 bg-white border-r border-slate-200 leading-5 hover:bg-slate-100 transition-colors" aria-label="Anterior">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                            </svg>
                        </a>
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                ...
                                @else
                                    <a href="{{ $url }}" class="inline-flex items-center px-3.5 py-1.5 text-xs font-semibold text-slate-700 bg-white border-r border-slate-200 leading-5 hover:bg-slate-100 transition-colors" aria-label="Ir a página {{ $page }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
                    ...
                    @if ($paginator->hasMorePages())
                        <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-slate-700 bg-white leading-5 hover:bg-slate-100 transition-colors" aria-label="Siguiente">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
