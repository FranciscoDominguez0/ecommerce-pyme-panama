...
@section('breadcrumbs')
    ...
    <a href="{{ route('admin.categorias.index') }}" class="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors truncate max-w-[85px] sm:max-w-none">Categorías</a>
    <span class="material-symbols-outlined text-[13px] text-slate-300 shrink-0">chevron_right</span>
    <span class="font-bold text-slate-900 dark:text-white truncate shrink-0">{{ $esEdicion ? 'Editar' : 'Nueva' }}</span>
@endsection
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-gray-700/80">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.categorias.index') }}{{ request()->getQueryString() ? '?' . request()->getQueryString() : '' }}" 
               class="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-slate-100 dark:bg-transparent rounded-lg transition-colors" 
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.categorias.index') }}{{ request()->getQueryString() ? '?' . request()->getQueryString() : '' }}" 
               class="px-4 py-2 border border-slate-200 dark:border-gray-700 text-slate-700 dark:text-slate-300 rounded-lg text-xs font-semibold hover:bg-slate-50 dark:bg-transparent dark:hover:bg-gray-700/30 transition-colors shadow-xs">
                Cancelar
            </a>
    ...
    <form id="categoria-form" 
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card-elevated rounded-xl p-5 sm:p-6 space-y-5">
                <div class="flex items-center gap-2.5 pb-3 border-b border-slate-100 dark:border-gray-700">
                    <span class="material-symbols-outlined text-[18px] text-emerald-600">category</span>
                    <h3 class="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Información General</h3>
                </div>

                <!-- Nombre -->
                <div class="space-y-1.5">
                    <label for="nombre" class="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative flex items-center">
                        ...
                        <input type="text" 
                               ...
                               class="w-full bg-slate-50 dark:bg-transparent border @error('slug') border-red-300 ring-2 ring-red-500/10 @else border-slate-200 dark:border-gray-700 @enderror rounded-lg pl-24 pr-3.5 py-2.5 text-xs text-slate-900 dark:text-white font-mono placeholder:text-slate-400 focus:bg-white dark:bg-[#181a1b] focus:border-slate-400 focus:ring-2 focus:ring-slate-900/5 transition-all outline-none" />
                    </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <div id="contenedor-padre-card" class="relative">
                        ...
                        <div id="card-padre-activo" class="hidden items-center justify-between p-2.5 bg-emerald-50/80 border border-emerald-200 rounded-xl hover:border-emerald-300 transition-all min-h-[48px]">
                            ...
                            <div class="flex items-center gap-1 shrink-0">
                                <button type="button" 
                                        ...
                                        class="px-2.5 py-1.5 text-xs font-semibold text-emerald-900 bg-white dark:bg-[#181a1b] hover:bg-emerald-100/60 border border-emerald-200 rounded-lg transition-colors cursor-pointer flex items-center gap-1 shadow-2xs">
                        ...
                        <button type="button" 
                                ...
                                onclick="abrirModalPadres()" 
                                class="w-full flex items-center justify-between px-3.5 py-2.5 bg-slate-50 dark:bg-transparent hover:bg-slate-100 dark:bg-transparent/80 border border-dashed border-slate-300 dark:border-gray-700 hover:border-slate-400 rounded-xl transition-all group text-left cursor-pointer min-h-[48px]">
                            <div class="flex items-center gap-2.5">
                                <div class="w-7 h-7 rounded-lg bg-slate-200 dark:bg-gray-700/60 text-slate-500 dark:text-slate-400 group-hover:text-slate-700 dark:text-slate-300 flex items-center justify-center shrink-0">
                                    <span class="material-symbols-outlined text-[16px]">folder_open</span>
                                </div>
                                <div>
                                    <span class="text-xs font-bold text-slate-700 dark:text-slate-300 group-hover:text-slate-900 dark:text-white block">— Ninguna (Categoría Principal / Raíz) —</span>
                                    <span class="text-[10px] text-slate-400 block font-normal">Haz clic para buscar y asignar una categoría padre</span>
                                </div>
                            </div>
                            <span class="px-2.5 py-1 rounded-lg bg-white dark:bg-[#181a1b] border border-slate-200 dark:border-gray-700 text-slate-700 dark:text-slate-300 text-xs font-bold flex items-center gap-1 shadow-2xs group-hover:border-slate-300 dark:border-gray-700">
                                <span class="material-symbols-outlined text-[14px] text-slate-500 dark:text-slate-400">search</span>
                                <span>Explorar</span>
                            </span>
                        </button>
                    </div>
                ...
                <!-- Descripción -->
                <div class="space-y-1.5">
                    <label for="descripcion" class="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider">
                    ...
                    <textarea id="descripcion" 
                              ...
                              class="w-full bg-slate-50 dark:bg-transparent border @error('descripcion') border-red-300 @else border-slate-200 dark:border-gray-700 @enderror rounded-lg px-3.5 py-2.5 text-xs text-slate-900 dark:text-white font-medium placeholder:text-slate-400 focus:bg-white dark:bg-[#121415] focus:border-slate-400 focus:ring-2 focus:ring-slate-900/5 transition-all outline-none resize-y">{{ old('descripcion', $categoria->descripcion) }}</textarea>
                    @error('descripcion')
                        <p class="text-[11px] text-red-600 font-medium flex items-center gap-1 mt-1">
                            <span class="material-symbols-outlined text-[14px]">error</span>
                            {{ $message }}
        ...
        <div class="space-y-6">
            ...
            <div class="card-elevated rounded-xl p-5 sm:p-6 space-y-5">
                <div class="flex items-center gap-2.5 pb-3 border-b border-slate-100 dark:border-gray-700">
                    <span class="material-symbols-outlined text-[18px] text-slate-700 dark:text-slate-300">tune</span>
                    <h3 class="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Estado y Publicación</h3>
                </div>

                <!-- Toggle Estado Activo -->
                <div class="p-3.5 rounded-lg bg-slate-50 dark:bg-transparent border border-slate-200 dark:border-gray-700/80 flex items-center justify-between gap-3">
                    <div>
                        <span class="text-xs font-bold text-slate-900 dark:text-white block">Categoría Activa</span>
                        <span class="text-[11px] text-slate-500 dark:text-slate-400 block mt-0.5">Visible en la tienda y catálogo</span>
                    </div>
                ...
                <div class="p-3.5 rounded-lg bg-slate-50 dark:bg-transparent border border-slate-200 dark:border-gray-700/80 flex flex-col gap-2.5">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            <div class="flex items-center gap-1.5">
                                <span class="material-symbols-outlined text-[16px] text-emerald-600">local_shipping</span>
                                <span class="text-xs font-bold text-slate-900 dark:text-white">Exento de Envío</span>
                            </div>
                            <span class="text-[11px] text-slate-500 dark:text-slate-400 block mt-0.5">No cobra flete (servicios, licencias o armados)</span>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" 
                                   name="exento_envio" 
                ...
                <div class="space-y-1.5">
                    <label for="orden_visualizacion" class="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider">
                        Orden de Visualización
                    </label>
                    <input type="number" 
                           ...
                           min="0" 
                           class="w-full bg-slate-50 dark:bg-transparent border border-slate-200 dark:border-gray-700 rounded-lg px-3.5 py-2.5 text-xs text-slate-900 dark:text-white font-semibold focus:bg-white dark:bg-[#181a1b] focus:border-slate-400 focus:ring-2 focus:ring-slate-900/5 transition-all outline-none" />
            ...
            <div class="card-elevated rounded-xl p-5 sm:p-6 space-y-4">
                <div class="flex items-center gap-2.5 pb-3 border-b border-slate-100 dark:border-gray-700">
                    <span class="material-symbols-outlined text-[18px] text-slate-700 dark:text-slate-300">image</span>
                    <h3 class="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Imagen de Portada</h3>
                </div>

                <!-- Preview Actual / Nuevo -->
                <div class="space-y-3">
                    <div id="image-preview-container" class="w-full h-44 rounded-xl bg-slate-50 dark:bg-transparent border-2 border-dashed border-slate-200 dark:border-gray-700 hover:border-slate-300 dark:border-gray-700 transition-colors flex flex-col items-center justify-center overflow-hidden relative group shadow-2xs">
                        ...
                        @else
                            ...
                            <div id="placeholder-icon" class="flex flex-col items-center text-slate-400 p-4 text-center">
                                <div class="w-12 h-12 rounded-full bg-slate-100 dark:bg-transparent flex items-center justify-center mb-2 text-slate-400">
                                    <span class="material-symbols-outlined text-[26px]">add_photo_alternate</span>
                                </div>
                                <span class="text-xs font-semibold text-slate-600 dark:text-slate-400">Sin imagen asignada</span>
                    ...
                    <div class="relative">
                        ...
                        <label for="imagen" 
                               class="w-full py-2 px-3 border border-slate-200 dark:border-gray-700 rounded-lg text-slate-700 dark:text-slate-300 font-semibold text-xs hover:bg-slate-50 dark:bg-transparent dark:hover:bg-gray-700/30 text-center transition-colors flex items-center justify-center gap-1.5 cursor-pointer shadow-xs">
                    ...
                    @if($categoria->imagen_ruta)
                        <div class="flex items-center gap-2 pt-1">
                            <input type="checkbox" id="eliminar_imagen" name="eliminar_imagen" value="1" class="rounded text-red-600 focus:ring-red-500 border-slate-300 dark:border-gray-700">
            ...
            @if($esEdicion)
                <div class="card-elevated rounded-xl p-5 space-y-3 bg-slate-50 dark:bg-transparent/50">
                    <div class="flex items-center justify-between text-xs">
                        <span class="text-slate-500 dark:text-slate-400 font-medium">Productos en esta categoría:</span>
                        <span class="font-bold text-slate-900 dark:text-white bg-white dark:bg-[#181a1b] px-2 py-0.5 rounded border border-slate-200 dark:border-gray-700">
                            {{ $categoria->productos()->count() }}
                        </span>
                    </div>
                    <div class="flex items-center justify-between text-xs">
                        <span class="text-slate-500 dark:text-slate-400 font-medium">Subcategorías dependientes:</span>
                        <span class="font-bold text-slate-900 dark:text-white bg-white dark:bg-[#181a1b] px-2 py-0.5 rounded border border-slate-200 dark:border-gray-700">
...
<div id="modal-padres-categoria" class="fixed inset-0 z-50 hidden items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs transition-opacity">
    <div class="bg-white dark:bg-[#181a1b] rounded-2xl border border-slate-200 dark:border-gray-700 shadow-2xl max-w-lg w-full p-6 space-y-4 transform transition-all flex flex-col max-h-[90vh]">
        
        <!-- Header -->
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-gray-700 pb-3.5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-slate-900 dark:text-white">Seleccionar Categoría Padre</h3>
                    <p class="text-[11px] text-slate-500 dark:text-slate-400">Busca y asigna la categoría superior jerárquica.</p>
                </div>
            </div>
            <button type="button" onclick="cerrarModalPadres()" class="text-slate-400 hover:text-slate-700 dark:text-slate-300 p-1.5 rounded-lg hover:bg-slate-100 dark:bg-transparent transition-colors">
        ...
        <div class="relative">
            ...
            <input type="text" 
                   ...
                   class="w-full bg-slate-50 dark:bg-transparent border border-slate-200 dark:border-gray-700 rounded-xl pl-10 pr-9 py-2.5 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 focus:bg-white dark:bg-[#121415] focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all outline-none">
            <button type="button" 
                    id="btn-limpiar-busqueda-padres" 
                    onclick="filtrarPadresModal('')" 
                    class="hidden absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:text-slate-400 p-1 rounded-md">
                <span class="material-symbols-outlined text-[16px]">close</span>
            </button>
        </div>
        ...
        <div class="pt-1">
            <div onclick="seleccionarPadre('', '— Ninguna (Categoría Principal / Raíz) —', '')"
                 ...
                 class="p-3 bg-slate-50 dark:bg-transparent hover:bg-slate-100 dark:bg-transparent/90 border border-slate-200 dark:border-gray-700/90 rounded-xl cursor-pointer transition-all flex items-center justify-between group">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg bg-white dark:bg-[#181a1b] border border-slate-200 dark:border-gray-700 text-slate-500 dark:text-slate-400 flex items-center justify-center shrink-0 shadow-2xs">
                        <span class="material-symbols-outlined text-[18px]">folder_open</span>
                    </div>
                    <div>
                        <span class="text-xs font-bold text-slate-900 dark:text-white block">— Ninguna (Categoría Principal / Raíz) —</span>
                        <span class="text-[11px] text-slate-500 dark:text-slate-400 block">Esta categoría se ubicará en el nivel superior.</span>
                    </div>
                </div>
        ...
        <div id="empty-state-padres" class="hidden text-center py-8 space-y-2">
            <div class="w-12 h-12 rounded-full bg-slate-100 dark:bg-transparent text-slate-400 flex items-center justify-center mx-auto">
                <span class="material-symbols-outlined text-[24px]">search_off</span>
            </div>
            <p class="text-xs font-semibold text-slate-700 dark:text-slate-300">No se encontraron categorías</p>
        ...
        <div class="flex items-center justify-between border-t border-slate-100 dark:border-gray-700 pt-3">
            ...
            <button type="button" onclick="cerrarModalPadres()" class="px-4 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-[#181a1b] border border-slate-200 dark:border-gray-700 rounded-xl hover:bg-slate-50 dark:bg-transparent dark:hover:bg-gray-700/30 transition-colors">
                Cerrar
            </button>
        </div>
    </div>
        ...
        lista.forEach(padre => {
            ...
            card.className = `p-3 rounded-xl border transition-all cursor-pointer flex items-center justify-between group ${
                isSelected 
                    ...
                    : 'bg-white dark:bg-[#181a1b] border-slate-200 dark:border-gray-700/80 hover:border-slate-300 dark:border-gray-700 hover:bg-slate-50 dark:bg-transparent dark:hover:bg-gray-700/30 hover:shadow-2xs'
            }`;
            ...
            card.innerHTML = `
                <div class="flex items-center gap-3 min-w-0">
                    <div class="w-8 h-8 rounded-lg ${isSelected ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 dark:bg-transparent text-slate-700 dark:text-slate-300'} flex items-center justify-center shrink-0 shadow-2xs">
                        <span class="material-symbols-outlined text-[18px]">${nivel > 0 ? 'subdirectory_arrow_right' : 'folder'}</span>
                    </div>
                    <div class="min-w-0">
                        <div class="text-xs font-bold ${isSelected ? 'text-emerald-950' : 'text-slate-800 dark:text-gray-100 group-hover:text-slate-900 dark:text-white'} truncate">
                            ${padre.nombre}
                        </div>
                        ${padre.padre_nombre ? `
                            <div class="text-[11px] text-slate-500 dark:text-slate-400 font-medium truncate flex items-center gap-1 mt-0.5">
                                <span class="material-symbols-outlined text-[13px] text-slate-400">schema</span>
                                <span class="text-slate-600 dark:text-slate-400 font-medium">Pertenece a: ${padre.padre_nombre}</span>
                            </div>
                        ` : `
