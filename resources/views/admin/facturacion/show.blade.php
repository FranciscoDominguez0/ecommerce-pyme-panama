...
<div class="space-y-6 font-sans">
    <div class="flex items-center justify-between mb-4">
        <a href="{{ route('admin.facturas.index') }}" class="inline-flex items-center text-sm font-medium text-primary dark:text-blue-400 hover:text-primary dark:text-blue-400/80">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @if($factura->estado === 'emitida')
                <form action="{{ route('admin.facturas.reenviar', $factura) }}" method="POST" x-data="{ sending: false }" @submit="sending = true">
                    ...
                    <button type="submit" class="inline-flex items-center px-4 py-2 bg-primary dark:bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-on-primary uppercase tracking-widest hover:bg-primary dark:bg-blue-600-container transition w-[130px] justify-center" :class="{ 'opacity-75 cursor-wait': sending }" :disabled="sending">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card-elevated rounded-xl p-6">
                <h2 class="text-lg font-bold text-slate-900 dark:text-white mb-4 border-b border-slate-100 dark:border-gray-700 pb-2">Artículos ({{ $factura->pedido->items->count() }})</h2>
            ...
            <div class="card-elevated rounded-xl p-6">
                <h2 class="text-lg font-bold text-slate-900 dark:text-white mb-6 border-b border-slate-100 dark:border-gray-700 pb-2">Historial de Reenvíos</h2>
                <div class="flow-root">
                                    ...
                                    <div class="relative flex space-x-3">
                                        <div>
                                            <span class="h-8 w-8 rounded-full bg-slate-100 dark:bg-transparent flex items-center justify-center ring-8 ring-white">
                                                <span class="material-symbols-outlined text-slate-500 dark:text-slate-400 text-sm">mail</span>
                                            </span>
                                        </div>
                                        <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                            <div>
                                                <p class="text-sm text-slate-900 dark:text-white font-medium">Reenviado a {{ $reenvio->email_destino }}</p>
                                                @if($reenvio->mensaje_personalizado)
                                                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">"{{ $reenvio->mensaje_personalizado }}"</p>
                                                @endif
                                            </div>
        ...
        <div class="space-y-6">
            <!-- Datos del Cliente -->
            <div class="card-elevated rounded-xl p-6">
                <h2 class="text-lg font-bold text-slate-900 dark:text-white mb-4 border-b border-slate-100 dark:border-gray-700 pb-2">Facturar a</h2>
                <div class="mb-4">
                    <p class="font-medium text-slate-900 dark:text-white text-sm">{{ $factura->usuario->nombre ?? 'Desconocido' }} {{ $factura->usuario->apellido }}</p>
                    <p class="text-sm text-slate-500 dark:text-slate-400">{{ $factura->usuario->email ?? '' }}</p>
                </div>
                
                @if($factura->pedido->direccionEnvio)
                <h3 class="text-sm font-bold text-slate-900 dark:text-white mb-2 mt-4">Dirección Comercial</h3>
                ...
                <div class="mt-4 pt-4 border-t border-slate-100 dark:border-gray-700">
                    <h3 class="text-sm font-bold text-slate-900 dark:text-white mb-2">Pedido Relacionado</h3>
                    <a href="{{ route('admin.pedidos.detalle', $factura->pedido->id) }}" class="inline-flex items-center text-sm text-primary dark:text-blue-400 hover:text-primary dark:text-blue-400/80 font-medium">
            ...
            <div class="card-elevated rounded-xl p-6">
                ...
                <dl class="space-y-3 text-sm text-slate-600 dark:text-slate-400">
                    <div class="flex justify-between">
                        <dt>Subtotal</dt>
                        <dd class="font-medium text-slate-900 dark:text-white">${{ number_format($factura->subtotal, 2) }}</dd>
                    </div>
                    ...
                    <div class="flex justify-between">
                        <dt>Envío</dt>
                        <dd class="font-medium text-slate-900 dark:text-white">${{ number_format($factura->costo_envio, 2) }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt>ITBMS ({{ $factura->itbms_tasa }}%)</dt>
                        <dd class="font-medium text-slate-900 dark:text-white">${{ number_format($factura->itbms_monto, 2) }}</dd>
                    </div>
                    <div class="flex items-center justify-between border-t border-slate-200 dark:border-gray-700 pt-3">
                        <dt class="text-base font-bold text-slate-900 dark:text-white">Total Facturado</dt>
                        <dd class="text-lg font-bold text-slate-900 dark:text-white">${{ number_format($factura->total, 2) }}</dd>
                    </div>
                </dl>
