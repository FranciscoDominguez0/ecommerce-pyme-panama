...
<div class="space-y-6 font-sans">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Gestión de Facturas</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Administra, descarga y reenvía las facturas generadas por el sistema.</p>
        </div>
    </div>
    ...
    <div class="card-elevated rounded-xl p-4 sm:p-5">
        <form action="{{ route('admin.facturas.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
            <div class="w-full sm:w-64">
                <label for="numero" class="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Buscar por N°</label>
                <input type="text" name="numero" id="numero" value="{{ request('numero') }}" placeholder="Ej: FAC-000001" class="block w-full rounded-md bg-white dark:bg-[#121415] text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-gray-500 border-slate-300 dark:border-gray-700 py-2 px-3 text-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500" onchange="this.form.submit()">
            </div>
            
            <div class="w-full sm:w-64">
                <label for="cliente" class="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Buscar por Cliente</label>
                <input type="text" name="cliente" id="cliente" value="{{ request('cliente') }}" placeholder="Nombre o email" class="block w-full rounded-md bg-white dark:bg-[#121415] text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-gray-500 border-slate-300 dark:border-gray-700 py-2 px-3 text-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500" onchange="this.form.submit()">
            </div>
            ...
            <div class="w-full sm:w-64">
                ...
                <select name="estado" id="estado" class="block w-full rounded-md bg-white dark:bg-[#121415] text-slate-900 dark:text-white border-slate-300 dark:border-gray-700 py-2 pl-3 pr-10 text-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500" onchange="this.form.submit()">
            ...
            <div class="w-full sm:w-48">
                <label for="emitida_desde" class="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Emitida desde</label>
                <input type="date" name="emitida_desde" id="emitida_desde" value="{{ request('emitida_desde') }}" class="block w-full rounded-md bg-white dark:bg-[#121415] text-slate-900 dark:text-white border-slate-300 dark:border-gray-700 py-2 px-3 text-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500" onchange="this.form.submit()">
            </div>

            <div class="w-full sm:w-48">
                <label for="emitida_hasta" class="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Emitida hasta</label>
                <input type="date" name="emitida_hasta" id="emitida_hasta" value="{{ request('emitida_hasta') }}" class="block w-full rounded-md bg-white dark:bg-[#121415] text-slate-900 dark:text-white border-slate-300 dark:border-gray-700 py-2 px-3 text-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500" onchange="this.form.submit()">
            </div>

            @if(request()->anyFilled(['estado', 'numero', 'cliente', 'emitida_desde', 'emitida_hasta']))
            <div class="flex items-end mb-1">
                <a href="{{ route('admin.facturas.index') }}" class="inline-flex items-center text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300">
                    ...
                    <tr class="hover:bg-slate-50 dark:bg-transparent dark:hover:bg-gray-700/30 transition-colors {{ $factura->estado === 'anulada' ? 'opacity-75' : '' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm font-medium {{ $factura->estado === 'anulada' ? 'text-slate-500 dark:text-slate-400' : 'text-slate-900 dark:text-white' }}">{{ $factura->usuario->nombre ?? 'Desconocido' }} {{ $factura->usuario->apellido }}</p>
                                    <p class="text-xs text-slate-500 dark:text-slate-400">{{ $factura->usuario->email ?? '' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium {{ $factura->estado === 'anulada' ? 'text-slate-500 dark:text-slate-400 line-through' : 'text-slate-900 dark:text-white' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($factura->estado === 'emitida')
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800">
                                    Emitida
                                </span>
                            @else
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border bg-slate-100 dark:bg-transparent text-slate-700 dark:text-slate-300 border-slate-200 dark:border-gray-700">
                                    Anulada
                                </span>
                            @endif
                        </td>
