...
@section('breadcrumbs')
    ...
    <span class="material-symbols-outlined text-[13px] text-slate-300 shrink-0">chevron_right</span>
    <span class="font-bold text-slate-900 dark:text-white truncate">Producto del Mes</span>
@endsection
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-[#181a1b] p-5 rounded-2xl border border-slate-200 dark:border-gray-700/80 shadow-2xs">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Destaca un producto estrella con precio promocional en la portada de la tienda.</p>
        </div>
    </div>
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-sm font-extrabold text-slate-900 dark:text-white">Producto Destacado del Mes</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400">Configura el producto con descuento especial de alto impacto.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div class="md:col-span-5 bg-white dark:bg-[#181a1b] border border-slate-200 dark:border-gray-700/80 rounded-2xl p-5 shadow-2xs flex flex-col justify-between">
            <div>
                <!-- Image Container with Discount Badge -->
                <div class="w-full h-56 bg-slate-50 dark:bg-transparent rounded-xl mb-4 overflow-hidden relative border border-slate-100 dark:border-gray-700 flex items-center justify-center">
                ...
                <h3 class="text-base font-extrabold text-slate-900 dark:text-white leading-tight mb-2" id="spotlight-titulo">
                    {{ $promocionActual->producto->nombre ?? 'Selecciona un Producto' }}
                </h3>

                <!-- Price display -->
                <div class="flex items-baseline gap-3 mt-4">
                    <span class="text-3xl font-extrabold text-slate-900 dark:text-white" id="spotlight-precio-promo">
            ...
            <!-- Status Indicator -->
            <div class="mt-6 pt-4 border-t border-slate-100 dark:border-gray-700 flex items-center justify-between text-xs">
                <span class="text-slate-500 dark:text-slate-400 font-medium">Estado de publicación:</span>
        ...
        <div class="md:col-span-7 bg-white dark:bg-[#181a1b] border border-slate-200 dark:border-gray-700/80 rounded-2xl p-6 shadow-2xs flex flex-col justify-between">
            <form action="{{ route('admin.promociones.producto-del-mes.guardar') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                <div class="border-b border-slate-100 dark:border-gray-700 pb-3 flex items-center justify-between">
                ...
                <div class="p-3 bg-slate-50 dark:bg-transparent border border-slate-200 dark:border-gray-700 rounded-xl flex items-center justify-between gap-3">
                    <div class="flex items-center gap-3 min-w-0">
                        <div class="w-10 h-10 rounded-lg bg-white dark:bg-[#181a1b] border border-slate-200 dark:border-gray-700 flex items-center justify-center shrink-0 shadow-2xs">
                            <span class="material-symbols-outlined text-slate-600 dark:text-slate-400">inventory_2</span>
                        </div>
                        <div class="min-w-0">
                            <span id="display-prod-nombre" class="text-xs font-bold text-slate-900 dark:text-white truncate block">
                                {{ $promocionActual->producto->nombre ?? 'Ningún producto seleccionado' }}
                            </span>
                            <span id="display-prod-precio" class="text-[11px] text-slate-500 dark:text-slate-400 font-medium block">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="descuento_especial" class="block text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase mb-1">
                        ...
                        <div class="relative">
                            <input type="number" 
                                   ...
                                   required 
                                   class="w-full bg-slate-50 dark:bg-transparent border border-slate-200 dark:border-gray-700 rounded-xl pl-3.5 pr-8 py-2.5 text-xs text-slate-900 dark:text-white font-bold focus:bg-white dark:bg-[#181a1b] focus:border-emerald-500 outline-none">
                            <span class="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 text-[18px]">percent</span>
                        </div>
                    </div>

                    <div>
                        <label class="block text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase mb-1">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="inicio_en" class="block text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase mb-1">Fecha de Inicio <span class="text-rose-500">*</span></label>
                        <input type="date" 
                               id="inicio_en" 
                               name="inicio_en" 
                               value="{{ old('inicio_en', $promocionActual && $promocionActual->inicio_en ? $promocionActual->inicio_en->format('Y-m-d') : now()->format('Y-m-d')) }}" 
                               required 
                               class="w-full bg-slate-50 dark:bg-transparent border border-slate-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 dark:text-white font-semibold focus:bg-white dark:bg-[#181a1b] focus:border-emerald-500 outline-none">
                    </div>

                    <div>
                        <label for="fin_en" class="block text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase mb-1">Fecha de Fin <span class="text-rose-500">*</span></label>
                        <input type="date" 
                               id="fin_en" 
                               name="fin_en" 
                ...
                <div>
                    <label for="descripcion_mes" class="block text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase mb-1">Frase Promocional / Leyenda del Mes</label>
                    <input type="text" 
                           id="descripcion_mes" 
                           ...
                           class="w-full bg-slate-50 dark:bg-transparent border border-slate-200 dark:border-gray-700 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 dark:text-white font-medium focus:bg-white dark:bg-[#181a1b] focus:border-emerald-500 outline-none">
                </div>

                <!-- Estado Switch -->
                <label class="flex items-center gap-3 cursor-pointer text-xs font-semibold text-slate-800 dark:text-gray-100 pt-2">
