...
@section('breadcrumbs')
    ...
    <span class="font-bold text-slate-900 dark:text-white truncate">Marcas</span>
@endsection
...
<div class="space-y-6 w-full min-w-0 max-w-full">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-gray-700/80">
        <div>
            <h2 class="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Gestión de Marcas</h2>
            <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-medium mt-0.5">Administra los fabricantes oficiales y logotipos vectoriales/imágenes del catálogo.</p>
        </div>
        ...
        <form method="GET" action="{{ route('admin.brands.index') }}" class="flex flex-col sm:flex-row gap-3 items-center justify-between">
            ...
            <div class="flex-1 w-full sm:w-auto relative">
                <div class="flex items-center bg-slate-50 dark:bg-transparent border border-slate-200 dark:border-gray-700 rounded-lg px-3.5 py-2 w-full focus-within:border-slate-400 focus-within:bg-white dark:bg-[#181a1b] focus-within:ring-2 focus-within:ring-slate-900/5 transition-all">
                    ...
                    @if(!empty($busqueda))
                        <a href="{{ route('admin.brands.index', ['verificada' => $filtroVerificada]) }}" class="text-slate-400 hover:text-slate-600 dark:text-slate-400">
    ...
    <div class="card-elevated rounded-xl overflow-hidden flex flex-col w-full min-w-0 max-w-full">
                        ...
                        <tr class="hover:bg-slate-50 dark:bg-transparent dark:hover:bg-gray-700/30 transition-colors group">
                            ...
                            <td class="py-3.5 px-4 sm:px-6 font-semibold text-slate-900 dark:text-white max-w-[280px]">
                                <div class="flex items-center gap-2.5 min-w-0">
                                    <span class="font-bold text-slate-900 dark:text-white text-sm truncate min-w-0" title="{{ $brand->name }}">{{ $brand->name }}</span>
                                </div>
                            </td>

                            <!-- Logotipo Preview -->
                            <td class="py-3.5 px-3 text-center">
                                <div class="w-20 h-9 mx-auto rounded-lg bg-white dark:bg-[#181a1b] border border-slate-200 dark:border-gray-700/80 shadow-2xs flex items-center justify-center p-1.5 overflow-hidden">
                                    {!! $brand->logo_html !!}
                                </div>
                            </td>
                            ...
                            <td class="py-3.5 px-4 text-center">
                                <code class="px-2 py-0.5 rounded bg-slate-100 dark:bg-transparent text-slate-600 dark:text-slate-400 font-mono text-[11px]">
                                    {{ $brand->slug }}
                                </code>
                            </td>
                            ...
                            <td class="py-3.5 px-4 text-center">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold {{ $brand->productos_count > 0 ? 'bg-indigo-50 text-indigo-700 border border-indigo-200' : 'bg-slate-50 dark:bg-transparent text-slate-400' }}">
                            ...
                            <td class="py-3.5 px-4 text-center">
                                ...
                                @else
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-slate-100 dark:bg-transparent text-slate-500 dark:text-slate-400 text-[11px] font-medium">
                                        Estándar
                                    </span>
                                @endif
                            </td>
                        ...
                        <tr>
                            <td colspan="6" class="py-12 px-6 text-center text-slate-400">
                                <div class="flex flex-col items-center justify-center max-w-sm mx-auto">
                                    <div class="w-12 h-12 rounded-full bg-slate-100 dark:bg-transparent flex items-center justify-center mb-3 text-slate-400">
                                        <span class="material-symbols-outlined text-[24px]">search_off</span>
                                    </div>
                                    <span class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">No se encontraron marcas</span>
        ...
        <!-- Paginación -->
        @if($marcas->hasPages())
            <div class="px-4 py-4 border-t border-slate-100 dark:border-gray-700 bg-slate-50 dark:bg-transparent/50">
