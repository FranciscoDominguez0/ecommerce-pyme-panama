...
<div class="space-y-6 font-sans">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Configuración de Impuestos</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Gestiona las tasas impositivas y cómo se muestran en la tienda.</p>
        </div>
    </div>
    ...
    <div class="card-elevated rounded-xl overflow-hidden mb-8">
        <div class="px-6 py-5 border-b border-slate-200 dark:border-gray-700 bg-white dark:bg-[#181a1b]">
            <h3 class="text-lg font-semibold text-slate-900 dark:text-white">Configuración de Impuestos</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Gestiona las tasas impositivas y cómo se muestran en la tienda.</p>
        </div>
            ...
            <div class="p-6 flex flex-col gap-8">
                ...
                <section>
                    <div class="flex justify-between items-center mb-4">
                        <h4 class="text-base font-semibold text-slate-900 dark:text-white">Impuesto General (ITBMS)</h4>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl">
                        ...
                        <div class="flex items-center justify-between gap-4 p-4 rounded-lg border border-slate-200 dark:border-gray-700 bg-white dark:bg-[#181a1b] hover:shadow-sm transition-shadow h-full">
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-slate-900 dark:text-white truncate">Aplicar ITBMS en compras</p>
                                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 break-words whitespace-normal">Habilita el cálculo del impuesto en el carrito y facturación.</p>
                            </div>
                            <div class="relative inline-block w-12 shrink-0 align-middle select-none transition duration-200 ease-in">
                                <input type="checkbox" name="itbms_activo" id="itbms_activo" value="1" class="toggle-checkbox absolute block w-6 h-6 rounded-full bg-white dark:bg-[#121415] border-4 appearance-none cursor-pointer border-slate-200 dark:border-gray-700 transition-all duration-300 z-10" {{ $itbmsActivo ? 'checked' : '' }}/>
                                <label for="itbms_activo" class="toggle-label block overflow-hidden h-6 rounded-full bg-slate-200 dark:bg-gray-700 cursor-pointer transition-colors duration-300"></label>
                            </div>
                        </div>
