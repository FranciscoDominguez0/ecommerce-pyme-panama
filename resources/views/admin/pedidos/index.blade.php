    ...
    <div class="card-elevated rounded-xl p-4 sm:p-5">
        <form action="{{ route('admin.pedidos.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
            <div class="w-full sm:w-64">
                ...
                <select name="estado" id="estado" class="block w-full rounded-md border-slate-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white py-2 pl-3 pr-10 text-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500" onchange="this.form.submit()">
            ...
            <div class="w-full sm:w-64">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="q" id="q" value="{{ request('q') }}" placeholder="N° Pedido o Cliente" class="block w-full rounded-md border-slate-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white py-2 pl-9 pr-3 text-sm focus:border-emerald-500 focus:outline-none focus:ring-emerald-500">
                </div>
            </div>
            ...
            <div class="flex items-end mb-1">
                <a href="{{ route('admin.pedidos.index') }}" class="inline-flex items-center text-sm text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-transparent divide-y divide-slate-100 dark:divide-gray-700/50">
                    ...
                    @endphp
                    <tr class="hover:bg-slate-50 dark:hover:bg-gray-700/50 transition-colors {{ $esNuevo ? 'bg-emerald-50/40 dark:bg-transparent border-l-4 border-l-emerald-500' : 'border-l-4 border-l-transparent' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                @else
                                    <div class="h-8 w-8 rounded-full bg-slate-200 dark:bg-gray-700 flex items-center justify-center text-slate-600 dark:text-slate-300 font-bold text-xs uppercase shrink-0">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php
                                $estadoClasses = [
                                    'pendiente' => 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700',
                                    ...
                                    'reembolsado' => 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-300 border-slate-300 dark:border-slate-600',
                                ];
                                $ultimoEstado = $pedido->ultimoEstado ? $pedido->ultimoEstado->estado : 'pendiente';
                                $claseEstado = $estadoClasses[$ultimoEstado] ?? 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700';
