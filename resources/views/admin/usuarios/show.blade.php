        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="card-elevated rounded-xl p-6 flex flex-col items-center text-center">
                @if($usuario->foto_perfil_ruta)
                    <div class="h-24 w-24 rounded-full bg-slate-200 dark:bg-gray-700 overflow-hidden border-4 border-white shadow-md mb-4">
                        <img alt="{{ $usuario->nombre }}" class="w-full h-full object-cover" src="{{ asset($usuario->foto_perfil_ruta) }}"/>
                    </div>
                @else
                    <div class="h-24 w-24 rounded-full bg-slate-100 dark:bg-transparent overflow-hidden border-4 border-white shadow-md mb-4 flex items-center justify-center text-slate-400 font-bold text-3xl uppercase">
                        {{ $usuario->iniciales }}
                    </div>
                @endif
                
                <h3 class="text-xl font-bold text-slate-900 dark:text-white">{{ $usuario->nombre_completo }}</h3>
                <p class="text-sm text-slate-500 dark:text-slate-400 mb-4">{{ $usuario->email }}</p>
                ...
                <div class="flex flex-wrap gap-2 justify-center mb-4">
                    ...
                    @foreach($usuario->roles as $rol)
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-slate-100 dark:bg-transparent text-slate-700 dark:text-slate-300 uppercase tracking-wide">
                            {{ $rol->name }}
                        </span>
                    @endforeach
                </div>
                
                <div class="w-full pt-4 border-t border-slate-100 dark:border-gray-700 flex flex-col gap-3 text-left">
            ...
            <div class="card-elevated rounded-xl p-6">
                <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-4 border-b border-slate-100 dark:border-gray-700 pb-2 flex items-center gap-2">
                    <span class="material-symbols-outlined text-slate-700 dark:text-slate-300">location_on</span>
                ...
                @if($usuario->direcciones->isEmpty())
                    <div class="p-6 text-center">
                        ...
                        <p class="text-sm text-slate-500 dark:text-slate-400 italic">No hay direcciones registradas.</p>
                    </div>
                @else
                    <div class="flex flex-col gap-4">
                        @foreach($usuario->direcciones as $direccion)
                            <div class="p-4 bg-slate-50 dark:bg-transparent rounded-lg border border-slate-100 dark:border-gray-700 relative">
                                @if($direccion->es_predeterminada)
                                    <span class="absolute top-3 right-3 inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-slate-200 dark:bg-gray-700 text-slate-700 dark:text-slate-300 uppercase tracking-wide">
                                        Principal
                                    </span>
                                @endif
                                <p class="text-sm font-semibold text-slate-900 dark:text-white flex items-center gap-1.5 mb-1">
                                    <span class="material-symbols-outlined text-[16px] text-slate-400">home</span>
                                    {{ $direccion->alias ?? 'Dirección' }}
                                </p>
                                <address class="text-xs text-slate-500 dark:text-slate-400 not-italic space-y-0.5 mt-2">
                                    <p class="text-slate-700 dark:text-slate-300 font-medium mb-1">{{ $direccion->nombre_receptor }}</p>
        ...
        <div class="lg:col-span-2 flex flex-col gap-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="card-elevated rounded-xl p-6 flex flex-col justify-center">
                    <span class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Total Pedidos</span>
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-12 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-600">
                            <span class="material-symbols-outlined text-[24px]">shopping_bag</span>
                        </div>
                        <span class="font-extrabold text-3xl text-slate-900 dark:text-white">{{ $totalPedidos }}</span>
                    </div>
                </div>
                
                <div class="card-elevated rounded-xl p-6 flex flex-col justify-center">
                    <span class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Total Gastado</span>
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-12 rounded-lg bg-emerald-50 flex items-center justify-center text-emerald-600">
                            <span class="material-symbols-outlined text-[24px]">payments</span>
                        </div>
                        <span class="font-extrabold text-3xl text-slate-900 dark:text-white">${{ number_format($totalGastado, 2) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="card-elevated rounded-xl overflow-hidden">
                <div class="p-6 border-b border-slate-100 dark:border-gray-700 flex justify-between items-center bg-slate-50 dark:bg-transparent">
                ...
                @if($usuario->pedidos->isEmpty())
                    <div class="p-12 text-center flex flex-col items-center">
                        <div class="w-16 h-16 bg-slate-50 dark:bg-transparent rounded-full flex items-center justify-center mb-4">
                            <span class="material-symbols-outlined text-slate-300 text-3xl">receipt_long</span>
                        </div>
                        <h3 class="text-base font-bold text-slate-900 dark:text-white mb-1">Sin historial de pedidos</h3>
                        <p class="text-sm text-slate-500 dark:text-slate-400 max-w-sm">Este usuario aún no ha realizado ninguna compra en la tienda.</p>
                    </div>
                @else
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-white dark:bg-[#181a1b] border-b border-slate-200 dark:border-gray-700">
                                    <th class="py-4 px-6 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">ID / Fecha</th>
                                    <th class="py-4 px-6 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Estado</th>
                                    <th class="py-4 px-6 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total</th>
                                    <th class="py-4 px-6 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-right">Acción</th>
                                </tr>
                            </thead>
                                        ...
                                        <td class="py-4 px-6">
                                            @php
                                                ...
                                                $estadoClasses = match(strtolower($estadoStr)) {
                                                    ...
                                                    'reembolsado' => 'bg-slate-100 dark:bg-transparent text-slate-800 dark:text-gray-100 border-slate-300 dark:border-gray-700 border',
                                                    default => 'bg-slate-100 dark:bg-transparent text-slate-700 dark:text-slate-300'
                                                };
                                            @endphp
