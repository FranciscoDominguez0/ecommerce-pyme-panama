<nav class="flex items-center gap-1 sm:gap-1.5 text-xs text-slate-500 font-medium min-w-0 flex-nowrap overflow-hidden" aria-label="Breadcrumb">
    <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-1 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 transition-colors shrink-0" title="Panel Principal">
    ...
    @else
        ...
        @if($segment2)
            ...
            @if($segment3 && $hasValidModuleRoute)
                ...
                <a href="{{ route($moduleRouteName) }}" class="capitalize font-medium text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 transition-colors truncate max-w-[90px] sm:max-w-none">
